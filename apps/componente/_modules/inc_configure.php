<?php

class Configure extends Ibe_Configure {

#    protected $filters = array();
#    protected $helpers = array();
#    protected $modules_params = array();
#    protected $action_return_json = TRUE;
#    protected $database_active = FALSE; /* NOT USED WITH DOCTRINE */
#    protected $database_host = 'mysql://root:123@localhost/teste'; /* DOCTRINE USE */
#    protected $database_host = "";
#    protected $database_user = "";
#    protected $database_pass = "";
#    protected $database_schm = "";
	 
	 public function getComponentList(){
	 	return array(
	 		"a"		
	 	);
	 }
}
